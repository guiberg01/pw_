...
export const resolveUploadContext = (req) => req.params.context;
...
const ensureReviewUploadEligibilityForCustomerOrThrow = async (subOrderId, userId) => {
  if (!subOrderId) {
    throw createHttpError(
      "subOrderId é obrigatório para upload de review",
      400,
      undefined,
      "UPLOAD_REVIEW_SUBORDER_REQUIRED",
    );
  }
...
export const authorizeUploadByContext = async (req) => {
  const context = resolveUploadContext(req);
...
export const buildUploadImageResponse = ({ req, file }) => {
  if (!file) {
